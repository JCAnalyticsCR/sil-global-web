...
(() => {
  ...
  const PRODUCTS = Object.freeze([
    ...
    { id: 'cinta-transparente', title: 'Cinta adhesiva transparente', category: 'cintas', image: 'tape.svg', summary: 'Presentaciones de 100 y 200 yardas', description: 'Cinta adhesiva transparente para empaque, en líneas comercial e industrial.', spec: 'Presentaciones publicadas: 100 y 200 yardas. Consultá la opción que necesitás.' },
    { id: 'bolsa-papel', title: 'Bolsas de papel', category: 'bolsas', image: 'paper.svg', summary: 'De ½ a 12 libras · Paquetes de 500+', description: 'Bolsas de papel para las necesidades de empaque de tu negocio.', spec: 'Tamaños publicados: de ½ libra a 12 libras. Paquetes de 500 unidades en adelante.' },
    ...
    { id: 'polipropileno', title: 'Bolsas de polipropileno', category: 'bolsas', image: 'clear-bag.svg', summary: 'Transparentes · Empaque y presentación', description: 'Bolsas de polipropileno transparentes para visualizar el contenido, empacar y almacenar productos.', spec: 'Indicá el tamaño y la cantidad que necesitás para consultar disponibilidad.' },
    { id: 'bolsa-negra', title: 'Bolsas negras', category: 'bolsas', image: 'black-bag.svg', summary: 'Uso doméstico, comercial e industrial', description: 'Bolsas negras para necesidades de uso doméstico, comercial e industrial.', spec: 'Consultá tamaño, presentación y cantidad según el uso que les darás.' },
    ...
    { id: 'lamina', title: 'Bolsas en lámina', category: 'bolsas', image: 'clear-bag.svg', summary: 'Una opción para el empaque de tu negocio', description: 'Bolsas en lámina incluidas en el catálogo publicado de SIL Global.', spec: 'Consultá las medidas, las presentaciones y la cantidad que necesitás.' },
    { id: 'rollo', title: 'Bolsas en rollo', category: 'bolsas', image: 'roll-bag.svg', summary: 'Presentación en rollo para empaque', description: 'Bolsas en rollo para las necesidades de empaque diario de tu negocio.', spec: 'Consultá el tamaño y la presentación adecuados para tu uso.' },
    { id: 'biodegradable', title: 'Bolsas biodegradables', category: 'bolsas', image: 'clear-bag.svg', summary: 'De ½ a 12 libras · Consultá características', description: 'Producto publicado bajo la denominación «bolsa biodegradable». Las propiedades y la certificación del material deben confirmarse antes de la compra.', spec: 'Tamaños publicados: de ½ a 12 libras. Consultá características y certificación.' },
    ...
    { id: 'cinta-pulgada', title: 'Cinta de empaque de 1 pulgada', category: 'cintas', image: 'tape.svg', summary: 'Uso industrial y comercial', description: 'Cinta de empaque de una pulgada para uso industrial y comercial.', spec: 'Medida publicada: 1 pulgada. Consultá presentación y cantidad.' }
  ]);
  ...
  function renderProducts() {
    ...
    shown.forEach((product) => {
      ...
      card.innerHTML = `<span class="product-image"><span class="product-category">${CATEGORY_SHORT[product.category]}</span><img src="${assetUrl(product.image)}" alt="Ilustración de ${escapeHtml(product.title)}" loading="lazy" width="440" height="330"></span><span class="product-body"><span class="product-title">${escapeHtml(product.title)}</span><span class="product-summary">${escapeHtml(product.summary)}</span><span class="product-bottom"><span>Consultar producto</span><span class="product-arrow">${icon('up-right')}</span></span></span>`;
  ...
  function openProduct(product) {
    ...
    $('#product-dialog-image').src = assetUrl(product.image);
    $('#product-dialog-image').alt = `Ilustración de ${product.title}; no es una fotografía del producto real.`;
  ...
  const sectionNav = [
    ['inicio', 'inicio'], ['soluciones', 'servicios'], ['servicios', 'servicios'], ['productos', 'productos'],
    ['acompanamiento', 'servicios'], ['internacional', 'servicios'], ['nosotros', 'nosotros'], ['contacto', null]
  ];
